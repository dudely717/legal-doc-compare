...
适用于 [Claude Code](https://claude.ai/code) 的法律文件对比分析 skill，面向非诉律师和法务人员。

## 功能简介

- **自动制备对比版**：仅提供前后两版净文本即可，自动生成 diff 标注版
- **修订登记**：区分实质修订、主体扩展（机制不变）、数据变更，强制前置登记防止遗漏
- **客户持仓识别**：自动识别客户在交易中的具体持仓/权利，确保所有优劣判断落实到客户具体利益
- **法律分析**：结合谈判立场（强势 / 中立 / 弱势）对每处实质修订出具风险等级与谈判建议
- **数据完整性核验**：自动核查文件内部数据一致性
- **矛盾条款核查**：跨文件比对，识别不同文件间的条款冲突
...
本 skill 在法律分析环节支持与使用者自备的谈判 Playbook 进行比对。如需启用：

1. 准备一份 Playbook 文件（Markdown 格式），记录你在各类条款上的标准立场与红线
2. 将文件放置在 `~/.claude/` 目录下
...
> "对比"、"比较"、"前后版本"、"修订了哪些"、"有什么变化"、"审阅（含多版本文件）"
...
- 投融资交易：投资协议、股东协议、公司章程跨版本对比
- 重组 / 上市：全套文件修订汇总分析
- 商业合同：采购、服务、合作协议版本对比
- 常法顾问：任何需要系统梳理两版文件差异的场景
